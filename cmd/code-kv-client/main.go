...
func kv(k, v string) *code.KeyValue {
	return &code.KeyValue{k, v}
}

func pat(pre, suf string) *code.Pattern {
	return &code.Pattern{pre, suf}
}

func kva(args []string) *code.KeyValue {
	if len(args) == 1 {
		return kv("", "")
	} else if len(args) == 2 {
		return kv(args[1], "")
...
func pata(args []string) *code.Pattern {
	if len(args) == 1 {
		return pat("", "")
	} else if len(args) == 2 {
		return pat(args[1], "")
...
func runCmd(s code.Storage, args []string) bool {
	var v string
	var b bool
	var lst code.List
	var n int
